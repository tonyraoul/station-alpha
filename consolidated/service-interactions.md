...
3. Out-of-order or duplicate ticks:
   enforce sequence checks at ingest and quarantine invalid events for reconciliation.
4. Historical Ticker Data API timeout:
   reduce chunk size, retry with backoff, and surface partial playback status in UI.
5. Trade metadata unavailable:
   return explicit replay-unavailable response and log for support investigation.
6. Archive retrieval delay:
   return accepted response with polling token for long-running restore operations.

### SLO and Observability Matrix

| Area                     | Target                          | Key Metrics                      | Alert Trigger          |
| ------------------------ | ------------------------------- | -------------------------------- | ---------------------- |
| Replay bootstrap latency | p95 < 300ms (hot path)          | p50/p95 bootstrap latency        | p95 > 500ms for 10m    |
| Replay chunk latency     | p95 < 200ms (hot path)          | chunk latency, cache hit rate    | p95 > 350ms for 10m    |
| Ingestion freshness      | < 2s lag                        | pub/sub consumer lag, ingest lag | lag > 5s for 5m        |
| Data integrity           | 0 unresolved gaps               | gap count, checksum failures     | any unresolved gap > 0 |
| Availability             | 99.9% monthly for BFF and APIs  | error rate, saturation           | 5xx > 1% for 5m        |
| Rollup quality           | > 99% cacheable window coverage | rollup hit rate, backfill rate   | hit rate < 85% for 15m |

Instrumentation baseline:

1. Trace IDs propagated from BFF to downstream APIs.
2. Structured logs with trade_id, symbol, and actor scope.
3. Dashboards for replay latency, cache performance, ingest lag, and integrity events.
