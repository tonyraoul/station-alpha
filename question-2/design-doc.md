...
2. Hydrate each trade into replay-ready compact segments (ordered, indexed, and checksumed).
3. Serve hydrated segments for replay by default to reduce query and rendering overhead.
...
3. Hydration compacts replay data by trade window, which lowers replay query cost.
4. Hot and cold tiers balance low-latency queries with storage cost efficiency.
5. Client replay logic is split into controller/store components for simpler maintenance.
6. A BFF layer tailors replay responses for web and mobile clients without leaking backend complexity.
...
2. The BFF can merge metadata + hydrated segments in one response to reduce round trips.
...
3. At rest (hot replay): hydrated compact segments optimized for replay reads.
...
1. Hot tier:
   Hydrated compact replay segments for fast replay and support tooling.
2. Cold tier:
...
3. Store full-fidelity ticks once, then hydrate to compact replay segments.
...
2. GET /core/replays/{trade_id}/segments?cursor=...&limit=...
3. GET /core/replays/{trade_id}/stream

BFF Replay API:

1. GET /bff/replays/{trade_id}/bootstrap
   Returns metadata + first hydrated segment + permissions in one call.
2. GET /bff/replays/{trade_id}/segments?cursor=...&limit=...
   Returns client-shaped segments and UI-ready annotations.
...
2. Segment-based chunking generated by hydration.
...
1. ReplayController:
   Loads metadata, orchestrates hydrated segment fetch, handles errors/retries.
2. ReplayStore:
   Holds immutable hydrated segments and integrity state.
...
3. Replay from hydrated compact segments; never mutate persisted source data.
...
1. Raw-only replay vs hydration replay:
   raw-only is simpler but expensive to query repeatedly; hydration improves replay latency and cost.
...
5. Hydration pipeline pattern:
   transform raw events into compact read models for low-latency replay.
